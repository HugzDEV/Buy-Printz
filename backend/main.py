...
@app.post("/api/orders/create-with-images")
async def create_order_with_images(
    order_data: dict,
    current_user: dict = Depends(get_current_user)
):
    """Create a new order with canvas images stored in Supabase"""
    try:
        print(f"🎨 Creating order with images for user: {current_user['user_id']}")
        print(f"🎨 Product type: {order_data.get('product_type')}")
        print(f"🎨 Has canvas_image: {bool(order_data.get('canvas_image'))}")
        print(f"🎨 Has surface_images: {bool(order_data.get('surface_images'))}")
        print(f"🎨 Surface images keys: {list(order_data.get('surface_images', {}).keys())}")
        print(f"🎨 Dimensions: {order_data.get('dimensions')}")
        print(f"🎨 Design option: {order_data.get('design_option')}")
        print(f"🎨 Tent design option: {order_data.get('tent_design_option')}")
        ...
        base_prices = {
            ...
            "sign": 35.00,
            "sticker": 15.00,
            "business_card_tin": 399.99,
            "tradeshow_tent": 899.99,
            "custom": 50.00
        }
        
        base_price = base_prices.get(order_data.get("product_type"), 50.00)
        ...
        total_amount = (base_price * quantity) + marketplace_cost
        
        # Create order payload with canvas images
        order_payload = {
            "product_type": order_data.get("product_type"),
            "quantity": quantity,
            "dimensions": order_data.get("dimensions"),
            "canvas_data": order_data.get("canvas_data"),
            "canvas_image": order_data.get("canvas_image"),  # Store canvas image
            "surface_images": order_data.get("surface_images"),  # Store surface images
            "surface_elements": order_data.get("surface_elements"),  # Store surface elements
            ...
            "total_amount": total_amount,
            "status": "pending",
            # Add design option for multi-surface products
        ...
        print(f"🎨 Order payload created, calling db_manager.create_order...")
        ...
        print(f"🎨 Database result: {order_result}")
        
        if order_result["success"]:
            print(f"🎨 Order created successfully: {order_result['order_id']}")
            return {
                "success": True,
                "order_id": order_result["order_id"],
                "total_amount": total_amount,
        ...
        print(f"🎨 Unexpected error in create_order_with_images: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
...
@app.get("/api/orders/{order_id}/images")
async def get_order_images(
    order_id: str,
    current_user: dict = Depends(get_current_user)
):
    """Get canvas images for a specific order"""
    try:
        ...
        # Extract images from order_details or fallback to canvas_data
        order_details = order.get("order_details", {})
        if isinstance(order_details, str):
            import json
            order_details = json.loads(order_details)
        
        # If order_details is empty, try to extract from canvas_data
        if not order_details:
            canvas_data = order.get("canvas_data", {})
            if isinstance(canvas_data, str):
                canvas_data = json.loads(canvas_data)
            order_details = canvas_data
        
        return {
            "success": True,
            "order_id": order_id,
            "canvas_image": order_details.get("canvas_image"),
            "surface_images": order_details.get("surface_images"),
            "surface_elements": order_details.get("surface_elements"),
            "canvas_data": order.get("canvas_data")
        }
