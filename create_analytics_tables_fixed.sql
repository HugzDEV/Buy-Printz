-- Create analytics and shipping tables with correct column references
-- This version uses the correct primary key column name from the users table

-- Template purchases table for tracking template sales
CREATE TABLE IF NOT EXISTS public.template_purchases (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    template_id UUID NOT NULL,
    user_id UUID NOT NULL, -- This will reference the primary key from users table
...
-- This will only work if the users table has the correct structure
INSERT INTO public.template_purchases (template_id, user_id, purchase_price)
SELECT
    ct.id,
    u.id, -- Using 'id' as the primary key from users table
    CASE
