-- BuyPrintz Creator Follower System Database Update
...
-- Add constraint for followers count (only if it doesn't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
CREATE TABLE IF NOT EXISTS creator_notifications (
    ...
    INDEX idx_creator_notifications_follower (follower_user_id, is_read, created_at),
    INDEX idx_creator_notifications_creator (creator_id, created_at)
);
...
CREATE INDEX IF NOT EXISTS idx_creator_notifications_follower_unread ON creator_notifications(follower_user_id, is_read) WHERE is_read = false;
CREATE INDEX IF NOT EXISTS idx_creator_notifications_creator_type ON creator_notifications(creator_id, notification_type);
...
-- Note: This would be populated through the application, not manually
-- But here's the structure for reference:

/*
-- Example of following a creator
INSERT INTO creator_followers (creator_id, follower_user_id) 
VALUES ('creator-uuid', 'follower-uuid');

-- Example of setting notification preferences
INSERT INTO creator_following_preferences (follower_user_id, creator_id, notify_new_templates, notify_sales, notify_updates)
VALUES ('follower-uuid', 'creator-uuid', true, false, true);
*/
