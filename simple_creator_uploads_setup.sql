-- SIMPLE CREATOR UPLOADS SETUP
-- Basic version without admin checking for now
...
-- Basic RLS policies (no admin checking for now)
...
-- Allow all authenticated users to view all uploads for now
-- (You can restrict this later when admin system is set up)
CREATE POLICY "Authenticated users can view all uploads" ON creator_uploads
